Here’s the todo list I’d use from where your game is now:
 <!-- Add a lives system — e.g. start with 3 lives, lose 1 on wrong answer, game over at 0. -->

 <!-- Add a game over state — stop accepting answers, show final score, provide restart. -->

- Add a restart button — reset score, lives, result text, input, and generate a new word.
- Prevent repeating the same word twice in a row.
